...
const Register = () => {
  ...
  const [_passwordAgain, _setPasswordAgain] = useState({
    ...JSON.parse(JSON.stringify(initialState)),
    rules: passwordValidationRules
  });
      ...
      errorKeys.forEach((e) => {
        switch(e) {
          ...
          case 'password':
            ...
            _setPassword(state);
            _setPasswordAgain(JSON.parse(JSON.stringify(state)));
            break;
        }
      });
    }
  ...
  const checkValidation = _username.validation.valid && _password.validation.valid && _passwordAgain.validation.valid;
  ...
  return (
    <div className={'animate__animated animate__slideInLeft container d-flex flex-fill justify-content-center justify-content-lg-start align-items-center'}>
      <div className='col-12 col-md-8 col-lg-5 col-xl-4 bg-white panel'>
        ...
        <div className={'panel-body mt-5'}>
          ...
          <div className={'col-12 mb-4'}>
          <CustomInput type={'password'} placeholder={'password...'} state={_password} setState={_setPassword} equalTo={{get: _passwordAgain, set: _setPasswordAgain}}/>
          </div>
          <div className={'col-12 mb-4'}>
          <CustomInput type={'password'} placeholder={'password again...'} state={_passwordAgain} setState={_setPasswordAgain} equalTo={{get: _password, set: _setPassword}}/>
          </div>
          <div className={'panel-footer d-flex justify-content-end'}>
          <Button disabled={(checkValidation) ? '' : 'disabled'} text={'Register'} icon={iconSVG} style={registerButtonStyle} classes={'solid-btn'} onClick={handleRegister}/>
          </div>
          </form>
        </div>
      </div>
